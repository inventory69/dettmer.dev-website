...
stack: Flutter, Dart, Material Design, REST API
---
...
Built this admin panel with Flutter. Figured if I'm gonna build it anyway, might as well make it cross-platform from the start.
Same codebase runs on Android, iOS, and Web. Write once, deploy everywhere - and it actually works this time.
...
- **Push notifications** - Control who gets notified about what. Granular opt-in/opt-out system
- **Theme toggle** - Switch between dark and light mode. Dark is default because we're not barbarians

## Tech Details

Flutter was the obvious choice. Write once, deploy everywhere. Material Design keeps the UI consistent across all platforms
without extra work.

State management with Provider. Keeps things simple, no unnecessary complexity. REST API calls with proper error handling
and retry logic.

Discord OAuth integration means users just click a button and they're authenticated. No signup forms, no password resets,
...
**Stack**: Flutter 3.35.7 · Dart · Material Design · Provider · REST API · Discord OAuth2 · Optimistic caching
